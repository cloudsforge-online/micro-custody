 ...
 * outbox — 273 rows, `custody.address.created` (263) and `custody.key.signed` (10):
 *
 *     as shipped -> malformed: version: missing
 *     fixed      -> well-formed; only the registration is outstanding
 *
 * Both live topics carry an actor and a correlation id, so `version` really was their only
 * defect. The two topics that ARE registered — `custody.export.requested` and
 * `custody.key.exported`, the pair `admin-api` subscribes to — have never been written, so
 * nothing has been lost at a subscriber yet. The ceremony is what breaks the day it is run.
 *
 ...
 *
 *   const key = await withOutbox(sql, SERVICE, async (tx, emit) => {
 *     const row = await insertKey(tx, input)
 *     emit({ topic: 'custody.address.created', key: row.id, payload: { id: row.id } })
 *     return row
 *   })
 *
...
export function buildEnvelope(row: OutboxRow): EventEnvelope {
  return {
    ...
    // `system` is the contract's own value for "no principal did this" — a scheduled key sweep, which
