...
# init container or a Kubernetes Job before this ever starts. `index.ts` asserts the schema version
# and refuses to serve below it, so a replica of the new code can never answer requests against the
# old schema.
#
# CUSTODY_DATA_DIR MUST BE A MOUNTED VOLUME, and it must be writable by uid 1000. Nothing else in
# this image is written to at runtime; that directory holds every key the platform custodies, so a
# container losing it loses the coins. It is also the reason this service does NOT run as root with
# a Docker socket mounted, which is what the service it supersedes does — see the README.
